...
nombres.sort()
PFCname = 'S029_0001665_00-Divertor_Assembly_Segment_2_Tile_4_Armor_Plates001'
...
for i,name in enumerate(nombres):
    if tag in name:
        outfile = root+name+'/openFoam/heatFoam/'+PFCname+'/postProcessing/fieldMinMax1/0/fieldMinMax.dat'
...
for i,df in enumerate(data):
    n = 'Psol = ' + namesWithTag[i].split('_')[-2]
    mask = df['field'] == 'T'
    t = df[mask].sort_values('# Time')['# Time'].values
    varMax = df[mask].sort_values('# Time')['max'].values
    varMax = np.insert(varMax, 0, 300)
    fig.add_trace(go.Scatter(x=t, y=varMax, name=n, line=dict(width=4,),
                         mode='lines', marker_symbol=symbols[i], marker_size=4,))
